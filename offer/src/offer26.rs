    ...
    pub fn sub_structure(a: Option<Rc<RefCell<TreeNode>>>, b: Option<Rc<RefCell<TreeNode>>>) -> bool {
        ...
        match a {
            Some(mut node) => {
                ...
                temp || Self::sub_structure( left_tree, b.clone()) || Self::sub_structure( right_tree , b)
            },
            None => {
                false
            }
    ...
    pub fn equal_structure(a: Option<Rc<RefCell<TreeNode>>>, b: Option<Rc<RefCell<TreeNode>>>) -> bool {
        match (a,b) {
            (Some(node1), Some(node2)) => {
                ...
                main_val == branch_val && Self::equal_structure(node1.borrow().left.clone(), node2.borrow().left.clone()) && Self::equal_structure(node1.borrow().right.clone(), node2.borrow().right.clone())
            },
            (Some(node1), None) => {
                 true
            },
